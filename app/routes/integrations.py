...
@router.post("/binance/connect", response_model=IntegrationResponse)
async def connect_binance(
    ...
    """
    Connect Binance by providing API Key and Secret.
    This will:
    1. Validate the API Key/Secret by calling Binance /api/v3/account
    2. Create a listenKey (optional) and store credentials encrypted
    ...
    # Usage gating (handle different return shapes from UsageService)
    from app.services.usage_service import UsageService
    usage_service = UsageService(db)
    try:
        can_use_res = usage_service.can_use_resource(business_id, "channel")
        # support coroutine or direct return
        if hasattr(can_use_res, "__await__"):
            can_use_res = await can_use_res

        if isinstance(can_use_res, tuple):
            can_add, reason = can_use_res
        else:
            can_add = bool(can_use_res)
            reason = None
    except Exception as e:
    ...
    if not can_add:
        raise HTTPException(
            status_code=status.HTTP_402_PAYMENT_REQUIRED,
            detail=(f"Channel limit reached. {reason}. Please upgrade your plan to add more channels." if reason else "Channel limit reached. Please upgrade your plan to add more channels.")
        )
    ...
    # Encrypt secret (requires env var)
    try:
        encrypted_secret = binance_encrypt_secret(request.api_secret)
    except Exception as e:
        log.error(f"Encryption key missing or invalid: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Server encryption key for storing credentials is not configured. Please set CREDENTIALS_ENCRYPTION_KEY in environment."
        )

    # Validate credentials with Binance
    try:
        account_info = await binance_test_credentials(request.api_key, request.api_secret)
    except httpx.HTTPStatusError as e:
        # Binance returns 401/403 for invalid signature/permissions
        log.error(f"Binance credential validation failed: {e}")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Failed to validate Binance credentials. Please check your API key and secret and ensure the key has correct permissions."
        )
    except Exception as e:
        log.error(f"Error validating Binance credentials: {e}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Failed to validate Binance credentials: {str(e)}"
        )

    # Optionally create listenKey (best-effort)
    listen_key = None
    try:
        listen_key = await binance_create_listen_key(request.api_key)
    except Exception:
        listen_key = None
    ...
    # Upsert ChannelIntegration
    existing = db.query(ChannelIntegration).filter(
        ChannelIntegration.business_id == business_id,
        ChannelIntegration.channel == "binance"
    ).first()

    if existing:
        existing.credentials = credentials
        existing.is_active = True
        existing.channel_name = request.channel_name or existing.channel_name or "Binance"
        existing.updated_at = datetime.utcnow()
        db.commit()
        db.refresh(existing)
        ...
        integration = ChannelIntegration(
            business_id=business_id,
            channel="binance",
            channel_name=request.channel_name or "Binance",
            credentials=credentials,
            is_active=True,
            webhook_url=None
        )
...
@router.get("/binance/status", response_model=IntegrationResponse)
async def get_binance_status(
    current_user: UserModel = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Get Binance integration status for the current business."""
    business_id = get_user_business_id(current_user, db)
    if business_id is None:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def test_binance_connection(
    ...
    """Test Binance connection by calling account endpoint and returning a small summary."""
    business_id = get_user_business_id(current_user, db)
    if business_id is None:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    if not integration:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Binance integration not found"
        )

    try:
        creds = json.loads(integration.credentials) if integration.credentials else {}
        api_key = creds.get("api_key")
        encrypted = creds.get("api_secret_encrypted")
        if not api_key or not encrypted:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid integration credentials")

        api_secret = binance_decrypt_secret(encrypted)
        account_info = await binance_test_credentials(api_key, api_secret)

        # Return a lightweight summary
        balances = account_info.get("balances", [])
        non_zero = [b for b in balances if float(b.get("free", 0)) != 0 or float(b.get("locked", 0)) != 0]
        return {"success": True, "assets_with_balance": len(non_zero)}

    except HTTPException:
        raise
    except Exception as e:
        log.error(f"Error testing Binance connection: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to test Binance connection"
        )
...
async def disconnect_binance(
    ...
    """Disconnect Binance integration (deactivate and remove listenKey)."""
    # Check role
    if current_user.role not in ["admin", "business_owner"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    # Try to remove listenKey if present (best-effort)
    try:
        creds = json.loads(integration.credentials) if integration.credentials else {}
        api_key = creds.get("api_key")
        listen_key = creds.get("listen_key")
        if api_key and listen_key:
            # Binance requires DELETE by sending the listenKey as param
            delete_url = f"https://api.binance.com/api/v3/userDataStream"
            async with httpx.AsyncClient() as client:
                await client.delete(delete_url, headers={"X-MBX-APIKEY": api_key}, params={"listenKey": listen_key}, timeout=10.0)
    except Exception:
        pass

    # Deactivate and clear sensitive fields
    try:
        creds = json.loads(integration.credentials) if integration.credentials else {}
        creds["listen_key"] = None
        creds["api_secret_encrypted"] = None
    ...
    log.info(f"Binance integration disconnected: {integration.id} by user {current_user.id}")

    return {"success": True, "message": "Binance integration disconnected successfully"}
